...
public class CoherenceBasedCacheManagerTests
    ...
    @Test
    public void testJCacheContainerActivation()
            throws URISyntaxException
        {
        final String CACHECONFIGFILE = "coherence-jcache-all-generated-cache-config.xml";

        // must be the ECCF Default URI so achieve by setting this system property.
        // only needed for this unit test, not needed when ContainerAdapter activates
        // ECCF using GAR configuration.
        System.setProperty(Constants.DEFAULT_COHERENCE_CONFIGURATION_URI_SYSTEM_PROPERTY, CACHECONFIGFILE);
        ...
        try
            ...
            simulateContainerAdapterActivation(eccf, CACHECONFIGFILE);

            CoherenceBasedCachingProvider provider = (CoherenceBasedCachingProvider) Caching.getCachingProvider();
            CacheManager                  mgr                 = provider.getCacheManager();
            CacheManager                  mgrContainerManaged = mgr;
            CoherenceBasedCacheManager    cmgr                = mgr.unwrap(CoherenceBasedCacheManager.class);

            assertTrue(cmgr.isContainerManaged());

            assertTrue(provider.isContainerContext());
            assertEquals("validate that LifeCycleEvent.ACTIVATED occurred", 1, interceptor.getActivatedCount());
            assertNotNull(eccf.getApplicationContext());
            assertEquals(CACHECONFIGFILE, eccf.getApplicationContext().getCacheConfigURI());
            assertEquals(cmgr.getConfigurableCacheFactory(), eccf);

            // validate JCache Container activation.
            assertTrue(interceptor.isContainerContext());
            assertNotNull(CoherenceBasedCacheManager.getContainerManagedCacheManager(eccf));

            // test creation of container unmanaged cache manager by using non-null uri parameter.
            CacheManager mgrUnmanaged = provider.getCacheManager(new URI("coherence-jcache-cache-config.xml"), null,
                                            null);
            CoherenceBasedCacheManager cmgrUnmanaged = mgrUnmanaged.unwrap(CoherenceBasedCacheManager.class);

            assertTrue(provider.isContainerContext());

            assertFalse(cmgrUnmanaged.isContainerManaged());
            assertNotEquals(eccf, cmgrUnmanaged.getConfigurableCacheFactory());
            assertNull(((ExtensibleConfigurableCacheFactory) cmgrUnmanaged.getConfigurableCacheFactory())
                .getApplicationContext());
            assertNull(CoherenceBasedCacheManager.getContainerManagedCacheManager(
                (ExtensibleConfigurableCacheFactory) cmgrUnmanaged.getConfigurableCacheFactory()));

            // validate getCacheManager with a non-null URI but the URI matches one in GAR that the returned CacheManager
            // is considered container-managed.
            CacheManager               mgr2  = provider.getCacheManager(new URI(CACHECONFIGFILE), null, null);
            CoherenceBasedCacheManager cmgr2 = mgr2.unwrap(CoherenceBasedCacheManager.class);

            assertTrue(cmgr2.isContainerManaged());
            assertEquals("expect only one container manager CacheManager independent of whether used default URI (null) or"
                         + " specified same URI as in GAR configuration", mgrContainerManaged, mgr2);

            // application must close container unmanaged cache manager.
            cmgrUnmanaged.getConfigurableCacheFactory().dispose();
            mgrUnmanaged.close();
            }
        finally
            {
            // simulate container deactivation resulting in disposing of eccf.
            if (eccf != null)
                {
                eccf.dispose();
                }

            cleanSimulateContainer();
            }
        }
