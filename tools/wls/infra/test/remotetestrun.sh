...
MVN_CMD_WITHOUT_DEBUG="$mvn_prompt -V -U -Dmaven.repo.local=$local_repo -f $DEV_ROOT/prj/pom.xml -e -B"
...
if [ $($JAVA_HOME/bin/java -version 2>&1 | sed 's/.*version "\([0-9]*\).*/\1/; 1q') -ge 17 ]; then
  export MAVEN_OPTS="--add-opens java.base/java.lang=ALL-UNNAMED --add-opens java.base/java.util.regex=ALL-UNNAMED --add-opens java.base/java.util=ALL-UNNAMED --add-opens java.base/java.text=ALL-UNNAMED --add-opens java.base/java.io=ALL-UNNAMED --add-opens java.base/java.nio.file=ALL-UNNAMED --add-opens java.base/java.lang.reflect=ALL-UNNAMED --add-opens java.desktop/java.awt.font=ALL-UNNAMED --add-opens java.base/java.time.format=ALL-UNNAMED --add-opens java.base/java.time=ALL-UNNAMED --add-opens java.base/sun.nio.fs=ALL-UNNAMED"
  MVN_TEST_CMD="${MVN_TEST_CMD} -Ptest-jdk17 "
fi
cd $DEV_ROOT/prj
...
for i in $TESTDATA; do
    ...
    # run javadoc in master job of remote.full, so no need to run build in master job
    if [ "$REMOTE_TEST" != "remote.full" -o "$PARALLEL" != "true" ]; then
        ...
        if [ "$build_target" = "build_installer" ]; then
           timebomb 7200 &
           timebomb_pid=$!
           (set -x ; env | sort ; ls -l $JAVA_HOME ; $MVN_CMD validate -P copyright && $MVN_CMD -DskipTests=true -Pcoherence,-modules clean install && $MVN_CMD clean install -P-coherence,modules,-javadoc -nsu -Dproject.official=true -Dproject.installer -DskipTests ) > ${TMPDIR:?}/$JOBID/build.log 2>&1
        elif [ -n "$STAGE_NAME" ]; then
          timebomb 600 &
          timebomb_pid=$!
          (set -x ; env | sort ; ls -l $JAVA_HOME ; $MVN_CMD validate -P copyright ) > ${TMPDIR:?}/$JOBID/build.log 2>&1
        else
           timebomb 2400 &
           timebomb_pid=$!
           (set -x ; env | sort ; ls -l $JAVA_HOME ; $MVN_CMD validate -P copyright && $MVN_CMD -Pcoherence,-modules clean install -DskipTests && $MVN_CMD -DskipTests -P-coherence,modules,-javadoc -nsu clean install) > ${TMPDIR:?}/$JOBID/build.log 2>&1
        fi
        saved_status=$?
    ...
    if [ "$test_target" != "" -a "$test_target" != "none" ];then
        ...
        if [ -n "$STAGE_NAME" ]; then
            ...
            # main job of remote.full
                ...
                test_name="javadoc"
                (set -x ; ls -l $JAVA_HOME ; env | sort ; cd $DEV_ROOT/prj && $MVN_CMD_WITHOUT_DEBUG -Dproject.official=true -DskipTests=true -Pcoherence,-modules clean install && $MVN_CMD_WITHOUT_DEBUG -Dproject.official=true -DskipTests=true -P-coherence,modules,javadoc -nsu -am -pl coherence-javadoc clean install) > ${TMPDIR:?}/$JOBID/${test_name}.log 2>&1
                javadoc_status=$?
                (set -x ; find $DEV_ROOT/prj/coherence-javadoc/target/invoker -type f | xargs tail -n50) >> ${TMPDIR:?}/$JOBID/${test_name}.log 2>&1
                publish ${TMPDIR:?}/$JOBID/$test_name.log
                if [ $javadoc_status -eq 0 ]; then
                  infomsg "test javadoc passed"
                  test_name=examples
                  infomsg "test examples starting..."
                  (set -x ; ls -l $JAVA_HOME ; env | sort ; cd $DEV_ROOT/prj && $MVN_CMD_WITHOUT_DEBUG -Dproject.official=true -DskipTests=true -Pcoherence,-modules clean install && $MVN_CMD_WITHOUT_DEBUG -Dproject.official=true -DskipTests=true -P-coherence,modules clean install -nsu && $MVN_CMD_WITHOUT_DEBUG -Pexamples clean install -nsu) > ${TMPDIR:?}/$JOBID/${test_name}.log 2>&1
                else
                  warnmsg "test javadoc failed" $test_name.log
                fi
            else
